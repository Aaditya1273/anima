...
The published package is [`@s0nderlabs/anima`](https://www.npmjs.com/package/@s0nderlabs/anima). The bin name is `anima`. Requires bun >=1.1. Current release is v0.26.0, pre-alpha. Always have the user run `anima --version` so you know exactly what surface you are working against.
...
- `anima --version`: confirm the user is on the surface you expect (currently v0.26.0).
