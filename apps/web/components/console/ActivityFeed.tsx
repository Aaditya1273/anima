  ...
  if (state.kind === 'loading') {
    return (
      <ActivityShell intro="Pulling the encrypted log from 0G Storage.">
        <p className="font-mono text-[12.5px] tracking-[0.04em] text-[var(--color-ink-3)]">
          Unsealing
        </p>
      </ActivityShell>
    )
...
function renderEntry(entry: ActivityEntry) {
  switch (entry.kind) {
    ...
    case 'tool-result': {
      ...
      return (
        <span className="block break-words font-mono text-[13.5px] leading-[1.55] text-[var(--color-ink-2)]">
          ...
          {d.name ?? 'tool'}{' '}
          <span
            className={ok ? '' : 'underline decoration-[var(--color-ink)] text-[var(--color-ink)]'}
          >
            {ok ? 'ok' : 'failed'}
          </span>
    ...
    case 'auto-topup': {
      ...
      return (
        <span className="flex w-full items-baseline gap-3 font-mono text-[13.5px] leading-[1.55]">
          ...
          <span
            className={`shrink-0 w-[64px] whitespace-nowrap text-[11px] tracking-[0.04em] ${
              status.tone === 'ok'
    ...
    default:
      return (
        <span className="font-mono text-[12.5px] tracking-[0.04em] text-[var(--color-ink-3)]">
          {entry.kind}
        </span>
      )
  }
    ...
    <div className="grid grid-cols-[56px_minmax(0,1fr)] items-baseline gap-5">
      ...
      <div className="min-w-0">
        ...
        >
          ...
          {hasResult && (
            <span
              className={`shrink-0 w-[64px] whitespace-nowrap font-mono text-[11px] tracking-[0.04em] ${
                ok
                  ? 'text-[var(--color-ink-3)]'
                  : 'text-[var(--color-ink)] underline decoration-[var(--color-border-strong)] underline-offset-2'
              }`}
              ...
              <div className="mt-3 grid gap-4 pb-2">
                ...
                {hasResult && result && (
                  <ExpandBlock
                    ...
                    body={ok ? asResultText(result.data) || '(empty)' : (result.error ?? 'failed')}
...
function ExpandBlock({
  ...
  return (
    <div className="grid grid-cols-[64px_minmax(0,1fr)] items-start gap-4">
      <p className="pt-[3px] font-mono text-[10.5px] tracking-[0.04em] text-[var(--color-ink-3)]">
        {label}
      </p>
      <pre
