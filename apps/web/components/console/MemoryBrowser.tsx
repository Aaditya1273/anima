  ...
  if (!files) {
    return (
      <div className="grid gap-3 pt-6">
        <span className="font-mono text-[11.5px] tracking-[0.04em] text-[var(--color-ink-3)]">
          Memory · Unsealing
        </span>
        ...
        <div className="flex flex-col gap-1">
          {viewingSibling ? (
            <button
              ...
              className="self-start font-mono text-[11px] tracking-[0.04em] text-[var(--color-ink-3)] transition-colors hover:text-[var(--color-ink)]"
              title={`Back to ${label}`}
            >
              ← {label}
            </button>
...
function PackedSiblingList({
  ...
  const visibleSiblings = needle ? siblings.filter(n => n.toLowerCase().includes(needle)) : siblings
