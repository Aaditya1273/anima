...
- **Web-app gateway: reach your on-chain agent from any browser.** A dumb reverse-tunnel relay (`@s0nderlabs/anima-relay`) plus a daemon-side client plugin (`@s0nderlabs/anima-plugin-webapp`) let the agent's daemon dial OUT to a relay, which pipes browser HTTP + SSE through the existing gateway contract (`/chat`, `/events`, `/approval/*`). No inbound port, no 127.0.0.1 step, no new dispatcher. Disabled by default; enable with the `webapp` plugin plus relay coordinates.
- **Operator console web chat.** The browser console gains a live chat surface: per-agent chat at `/console/<tokenId>/chat`, an agent-agnostic `/console/chat` landing (drops you on your last or first agent, switch agents inline), per-agent server-backed sessions, full mobile support (slide-in rail drawer), and a "Chat" nav entry. Writes are gated by a per-message EIP-191 signature or a device token; reads by SIWE plus the secret routing slug plus optional SSE read-auth; a server-side ownerOf gate fronts everything.
- **Device-token write-auth** ("sign once, then chat"): grant an opaque bearer for a chosen lifetime instead of signing every message. Operator-bound, revocable, and replay-guarded; does not survive an iNFT transfer.

### Fixed

- **Relay no longer resets long turns.** Tunneled browser requests disable Bun's per-connection idle timeout (`srv.timeout(req, 0)`), so a turn with an approval round-trip or slow inference (over 120s) is no longer reset mid-request; the relay's own request timeout stays the real bound.
- **0G upload bounded by a wall-clock timeout** so a reachable-but-stuck storage node can no longer hang `anima init` / `anima sync` indefinitely.

### Changed

- **Device tokens default to a finite 30-day ceiling** (was unbounded) so an out-of-the-box never-expire bearer cannot be minted; override via `ANIMA_WEBAPP_MAX_SESSION_TTL_MS`.
- Two new published packages (`@s0nderlabs/anima-relay`, `@s0nderlabs/anima-plugin-webapp`); nine `@s0nderlabs/anima-*` packages now ship per release. `release.yml` gains a pre-publish per-package version-skew guard.

## [0.25.0] - 2026-06-18

### Added

- **Opt-in 0G Router brain** as a second inference door, behind a provider-agnostic `CompletionTransport` seam. The default Direct path (0G serving-broker, EIP-712 per-request vouchers, drainable on-chain ledger) is unchanged and stays the default; setting `brain.providerKind: 'router'` routes inference through the OpenAI-compatible `router-api.0g.ai/v1` gateway instead.
- **Sovereign Router auth.** The agent self-signs a headless SIWE message with its own private key to mint its own `mk-`/`sk-` keys (no human-dashboard key). The `sk-` is sealed under an operator-signature scope (sibling encrypted blob, same pattern as the telegram secret), regenerable, and never written on-chain.
- **`anima topup --router N`** funds the prepaid Router Payment Layer credit (mainnet only). Guarded so a Direct agent or a testnet config is rejected before any signer loads.
- **Router model selection** at `anima init` / `anima model`, with Router-aware status and funding guidance. Router agents skip the compute-ledger prompt (they fund credit instead).

### Changed

- `OGComputeBrain` now dispatches through `DirectTransport` (default) or `RouterTransport`; `resolveProviderKind(brain)` returns `'router'` only on an explicit `providerKind: 'router'`, else `'direct'`. The Direct completion path is byte-identical to before the seam.
- `redactEnv` now denies `OG_ROUTER_*` (Bearer `sk-`/`mk-`, wallet JWT) so Router credentials never reach a tool environment.

### Safety

- Router Payment Layer deposits are non-recoverable, so every money path is conservative: auto-topup is explicit opt-in (`economy.autoTopup.enabled`), deposits wait for the receipt with an in-flight guard against double-deposit on a stale balance, sub-minimum/NaN amounts are rejected before any RPC, credit errors are scoped to 4xx, and Router is hard-refused in 0G Sandbox provisioning.

### Tests

- 1366/1366 unit tests pass, 124/124 forge tests pass, typecheck clean, lint clean.
- Live verified: `$0` Router inference, a live mainnet Payment Layer deposit, and an `anima init` Router smoke. Direct path covered by the unchanged `og-compute` suite. specter (#4) + enigma (#6) untouched.

[0.26.0]: https://github.com/s0nderlabs/anima/releases/tag/v0.26.0
[0.25.0]: https://github.com/s0nderlabs/anima/releases/tag/v0.25.0
